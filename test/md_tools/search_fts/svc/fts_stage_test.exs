defmodule MdTools.SearchFts.Svc.FtsStageTest do

  use ExUnit.Case

  alias MdTools.Doc.Svc.DocStage
  alias MdTools.Cache.Svc.JsonStage
  alias MdTools.SearchFts.Svc.FtsStage
  ...
  describe "start_link/1" do
    ...
    test "starts the GenServer" do
      setup()
      assert {:ok, _pid} = FtsStage.start_link(base_dir: @dir_base)
    end

    test "with start_supervised" do
      setup()
      assert {:ok, _pid} = start_supervised({FtsStage, [base_dir: @dir_base]})
    end

    test "with start_supervised!" do
      setup()
      assert start_supervised!({FtsStage, [base_dir: @dir_base]})
    end

    test "registered process name" do
      setup()
      start_supervised({FtsStage, [base_dir: @dir_base]})
      assert Process.whereis(:fts_stage)
    end

    test "with no arguments" do
      setup()
      start_supervised({FtsStage, []})
      assert Process.whereis(:fts_stage)
    end
  ...
  describe "subscribe" do
    ...
    test "link from consumer to producer with dir_base" do
      ...
      {:ok, consumer} = FtsStage.start_link()
